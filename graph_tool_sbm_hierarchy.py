...
with open("cache/MCMC/blocks/fdr-1e-5/example.dill", "wb") as f:
    dill.dump((g,state), f)
...
blocks = state.levels[0].get_blocks()  #returns a mapping of each vertex to its assigned block (at top level), based on the fitted (SBM) stored in the BlockState object state
...
gt.draw_hierarchy(state,
              ...
              output="graph_final_hierarchy.png")  # save to file


gt.draw_hierarchy(state,
              output_size=(2000, 2000),
              output="graph_final_hierarchy.svg")  # save to file