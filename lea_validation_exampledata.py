...
### Load the graph
g = gt.load_graph("network_trimmed.xml.gz")

### Load the optimized SBM block state
with open("sbm_optimized.dill", "rb") as f:
    state = dill.load(f)
...
gene_blocks = pd.read_csv("gene_block.csv")  

### Load block summary 
block_summary = pd.read_csv("block_summary.csv") 
